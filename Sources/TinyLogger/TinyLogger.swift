...
public extension TinyLogger {
    class TinyLoggerMain {
        ...
        public func debug(msg: String, format: String = "%t [%f/%T]: %m", dateFormat: String = "HH:mm:ss", file: String = #file, function: String = #function) {
            let date = Date() // Gets the date
            let formatter = DateFormatter()
            formatter.dateFormat = dateFormat
            fputs(format.replacingOccurrences(of: "%t", with: formatter.string(from: date)).replacingOccurrences(of: "%f", with: URL(fileURLWithPath: file).lastPathComponent.dropLast(6)).replacingOccurrences(of: "%T", with: "DEBUG").replacingOccurrences(of: "%F", with: function).replacingOccurrences(of: "%m", with: msg + "\n"), stdout) // Prints the final message to stdout
            fflush(stdout)
        }
...
public struct TinyLogger {
    public static let log = TinyLoggerMain() // Reference to TinyLogger class
}
